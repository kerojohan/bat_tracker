...
- `tracking.*`: distancia maxima de asociacion, tolerancia a frames perdidos y filtros minimos por trayectoria
  ...
  - `tracking.auto_scale_with_resolution`: si esta activo, escala linealmente los umbrales en pixeles del bloque `tracking` (`max_distance`, `valid_region_gate_dilate_px`, `min_track_displacement`, `min_track_path_length`, `merge_*distance*`, `max_spatial_distance_px`, `rescue_motion_min_*`, `rescue_motion_interaction_dilate_px`, etc.) segun la diagonal del video respecto a `reference_width`/`reference_height`
  - `tracking.reference_width` y `tracking.reference_height`: resolucion de referencia para los valores en pixeles del bloque `tracking`
  - `tracking.static_noise_filter_enabled`: activa el detector de "blob estatico" que descarta ruido fijo de larga duracion (reflejos, puntos calientes, vegetacion casi inmovil) que el tracker mantiene vivo y que acumula desplazamiento por saltos esporadicos (motivo de rechazo: `static_noise`). Solo rechaza un track cuando coinciden tres evidencias independientes a la vez, de modo que un murcielago real (rapido, o que avanza de verdad, o de duracion corta) no cae en el filtro. Los umbrales `static_noise_max_*_ratio_per_sec` ya estan normalizados por la diagonal del frame (no son pixeles fijos)
  ...
  - `tracking.static_noise_min_static_fraction`: segundo disparador del filtro `static_noise` (independiente del de velocidad). Si un track de duracion `>= static_noise_min_duration_sec` esta estatico en al menos esta fraccion de sus pasos entre detecciones consecutivas, se rechaza como `static_noise`. Captura blobs fijos que el tracker mantiene vivos y que de vez en cuando "teletransportan" (saltos grandes esporadicos que inflan la velocidad media y burlan el test de velocidad), pero que estan quietos la inmensa mayoria de frames; un murcielago en vuelo se mueve de forma continua y nunca esta estatico tanto tiempo. Con `0` se desactiva este disparador (default global: `0.0` desactivado para no alterar el comportamiento base; en `config.out3_clean.yaml` se activa con `0.80`)
  - `tracking.static_noise_static_step_ratio_per_frame`: umbral de movimiento por frame (como fraccion de la diagonal, por lo que escala con la resolucion) por debajo del cual un paso entre detecciones consecutivas se considera "estatico" para `static_noise_min_static_fraction`; se normaliza por los frames perdidos entre detecciones (default: `0.0005`)
  - `tracking.max_track_internal_gap_frames`: hueco temporal interno maximo permitido (en frames, no pixeles) entre dos detecciones consecutivas de un mismo track (motivo de rechazo: `temporal_gap`). Un track real es temporalmente denso (los huecos no superan `max_missed` ni `merge_max_gap_frames`); un hueco interno mucho mayor delata que se han cosido fragmentos no relacionados (p.ej. un vuelo corto + un punto fijo de esquina + una rafaga de ruido) bajo un mismo `track_id`, lo que infla `path_length` y burla el resto de filtros. Al estar en frames es independiente de la resolucion espacial. Con `0` se desactiva el filtro (default: `45`)
  - `tracking.loiter_filter_enabled`: activa el filtro de "merodeo" (motivo de rechazo: `loiter`). El vuelo de un murcielago saliendo de la cueva es un transito rapido que cruza la escena, no permanece muchos segundos en ella. Un track de larga duracion que NO transita (avance neto pequeno respecto al tamano del frame, aunque acumule mucho recorrido dando vueltas) no corresponde a ese comportamiento y suele ser ruido persistente (insecto cerca del objetivo, reflejo movil, objeto fijo). Distingue estos casos de las salidas reales rapidas, que si cruzan la escena
  - `tracking.loiter_min_duration_sec`: duracion (s) a partir de la cual un track se considera "largo" y candidato a merodeo (default: `10.0`; la mediana de duracion observada es ~0.6 s y el p99 ~5 s)
  - `tracking.loiter_min_displacement_ratio`: desplazamiento neto minimo (como fraccion de la diagonal del frame, por lo que escala con la resolucion) que un track largo debe tener para considerarse un transito real; por debajo de este valor el track largo se rechaza como `loiter` (default: `0.20`)
